...
\begin{figure}[hbt!]
  ...
  \begin{tikzpicture}[>=stealth]
    ...
    \node[compiler] (parser) [right=of input] {Parser};
    \node[language] (parse tree) [right=of parser] {Parse Tree};
    \node[compiler] (tree generator) [right=of parse tree] {Tree Generator};
    \node[language] (ast) [right=of tree generator] {AST};

    \draw[->] (input) -- (parser);
    \draw[->] (parser) -- (parse tree);
    \draw[->] (parse tree) -- (tree generator);
    \draw[->] (tree generator) -- (ast);
  \end{tikzpicture}
  \caption{Parser flow}
