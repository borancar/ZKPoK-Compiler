 ...
 *      Author: The compiler: verifier.c
 */
...
int init_verifier(struct cace_type *common_types[], struct cace_int *common_var[]) {
    int res = 0;
    ...
    IFNOERROR(cace_int_init(common_var[16], common_types[3]);); //var=_t_1-2/_t_1 group=ZMULT_p
    IFNOERROR(cace_int_init(common_var[17], common_types[4]);); //var=_c-2/_c group=_C
...
int Round1(struct cace_type *common_types[], struct cace_int *common_var[]) {
    ...
    IFNOERROR(cace_int_init(common_var[17], common_types[4]);); //var=_c-2/_c group=_C
    struct input_request input[] = {
        {"_t_1", common_var[16] }, // _t_1
    };
    IFNOERROR(request_comm(input, ARRAY_SIZE(input),1););
    MSG_INFO(fprintf(stderr,"in: _t_1 = ");cace_int_print_err(common_var[16]);fprintf(stderr,"\n"););
    IFNOERROR(cace_calc_element(NULL,common_var[16],CHECK_MEMB, NULL););
    //_c = RANDOM (Z, NULL)
    IFNOERROR(cace_calc(common_var[17],common_types[4],RANDOM,NULL););
    MSG_INFO(fprintf(stderr,"assign: _c = ");cace_int_print_err(common_var[17]);fprintf(stderr,"\n"););
    //send result
    ...
    if(send == NULL) res = -23;
    //send _c-2 _c 0
    MSG_INFO(fprintf(stderr,"out: _c = ");cace_int_print_err(common_var[17]);fprintf(stderr,"\n"););
    IFNOERROR(cace_int_serialize(send,common_var[17]););
    IFNOERROR(send_comm(send,cace_int_size(common_var[17])););
...
int Round2(struct cace_type *common_types[], struct cace_int *common_var[]) {
    ...
    IFNOERROR(cace_int_init(common_var[18], common_types[5]);); //var=_s_1-2-Round2/_s_1 group=ZPLUS_q
    struct input_request input[] = {
        {"_s_1", common_var[18] }, // _s_1
    };
    IFNOERROR(request_comm(input, ARRAY_SIZE(input),1););
    MSG_INFO(fprintf(stderr,"in: _s_1 = ");cace_int_print_err(common_var[18]);fprintf(stderr,"\n"););
    //ZPLUS_q (q) = q
    IFNOERROR(cace_type_assign_param(common_types[5],common_var[1]););
    IFNOERROR(cace_calc_element(NULL,common_var[18],CHECK_MEMB, NULL););
    //NULL = _s_1 CHECK_MEMB NULL
    IFNOERROR(cace_calc_element(NULL,common_var[18],CHECK_MEMB,NULL););
    //t2 = y POW _c
    IFNOERROR(cace_calc_element(t2,common_var[3],POW,common_var[17]););
    IFNOERROR(cace_int_push(t2););
    //t1 = _t_1 MULT t2
    IFNOERROR(cace_int_pop(t2););
    IFNOERROR(cace_calc_element(t1,common_var[16],MULT,t2););
    IFNOERROR(cace_int_push(t1););
    //t2 = g POW _s_1
    IFNOERROR(cace_calc_element(t2,common_var[4],POW,common_var[18]););
    IFNOERROR(cace_int_push(t2););
    //NULL = t1 VERIFY_EQUALS t2
    IFNOERROR(cace_int_pop(t2););
    IFNOERROR(cace_int_pop(t1););
    IFNOERROR(cace_calc_element(NULL,t1,VERIFY_EQUALS,t2););
    return res;
}
 ...
 */
int (*fpnt[])(struct cace_type *common_types[], struct cace_int *common_var[]) = { init_verifier, Round0, Round1, Round2 };
...
int main(void) {
	...
	if(get_var(&port_name, file_options, "term:port") == 0) {
	  strcat(port_name, "_verifier");
	}

	MSG_INFO(fprintf(stderr, "Adding verifier specific: %s!\n", port_name);)
